# tiny_llm_1b.py — ~998M param GPT | Colab Pro A100-80GB | Rung 5
import torch, torch.nn as nn
from torch.nn import functional as F
from torch.utils.checkpoint import checkpoint

block_size, n_embd, n_head, n_layer = 128, 1520, 16, 36
lr, steps, batch_size               = 2e-4, 5000, 8
device = "cuda"
...
class Block(nn.Module):
    def __init__(self):
        super().__init__()
        self.ln1=nn.LayerNorm(n_embd); self.attn=nn.MultiheadAttention(n_embd,n_head,batch_first=True,bias=False)
...
class TinyGPT(nn.Module):
    ...
    def forward(self,idx,targets=None):
        ...
        return logits, (F.cross_entropy(logits.view(-1,vocab_size),targets.view(-1)) if targets is not None else None)
    @torch.no_grad()
    def generate(self,idx,n):
        self.eval()
        for _ in range(n):
            logits,_=self(idx[:,-block_size:]); idx=torch.cat([idx,torch.multinomial(F.softmax(logits[:,-1],dim=-1),1)],dim=1)
        return idx

model=TinyGPT().to(device)
params=sum(p.numel() for p in model.parameters())
print(f"device={device} | dtype={dtype} | vocab={vocab_size} | parameters={params:,}")
