# tiny_llm_3b.py — ~3B param GPT | Colab Pro A100-80GB | Rung 7
import torch, torch.nn as nn
from torch.nn import functional as F
from torch.utils.checkpoint import checkpoint

block_size, n_embd, n_head, n_layer = 128, 3952, 16, 16
lr, steps, batch_size               = 5e-5, 5000, 8
device = "cuda"
...
bf16_state={k:v.to(torch.bfloat16) if v.is_floating_point() else v for k,v in model.state_dict().items()}
torch.save({"model":bf16_state,"stoi":stoi,
            "config":dict(block_size=block_size,n_embd=n_embd,n_head=n_head,n_layer=n_layer),
            "dtype":"bfloat16"},"tiny_llm_3b.pt")
size=__import__("os").path.getsize("tiny_llm_3b.pt")/1e9
print(f"saved tiny_llm_3b.pt ({size:.2f}GB)")
